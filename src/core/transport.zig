...
const ScriptedServer = struct {
    ...
    fn start(io: std.Io, replies: []const []const u8) !ScriptedServer {
        return startOn(io, .{ .ip4 = .loopback(0) }, replies);
    }

    fn startOn(io: std.Io, address: net.IpAddress, replies: []const []const u8) !ScriptedServer {
    ...
    fn deinit(s: *ScriptedServer, io: std.Io) void {
        s.server.deinit(io);
    }

    fn url(s: *const ScriptedServer, buf: []u8, path: []const u8) []const u8 {
        return std.fmt.bufPrint(buf, "http://127.0.0.1:{d}{s}", .{ s.port, path }) catch unreachable;
    }

    fn request(s: *const ScriptedServer, index: usize) []const u8 {
        return s.seen[index][0..s.seen_len[index]];
    }

    /// Serves every scripted reply, then returns.
    fn run(s: *ScriptedServer, io: std.Io) !void {
