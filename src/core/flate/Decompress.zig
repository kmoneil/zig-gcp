//! `std.compress.flate.Decompress` from Zig 0.16.0, with two changes, for
//! input that ends partway through a code, as a truncated download's does.
//! It is Zig's code, Copyright (c) Zig contributors, under the MIT license,
//! as this package is; `token.zig` beside it is copied unchanged.
//!
//! std's `tossBitsShort` adds the bits already consumed to the bits left
//! rather than taking them away, so near the end of the input it tosses a
//! code longer than what remains, tossing the last byte while bits of it
//! are still counted. `peekBitsEnding` then computes `left.len * 8 -
//! consumed_bits` with nothing left, which underflows: a panic in a safe
//! build, and a read past the input in `ReleaseFast`. The nightly fuzzing
//! found it with 19 bytes of gzip that end partway. `tossBitsShort` here
//! takes the consumed bits away, which is the fix; `peekBitsEnding` is also
//! written so it cannot underflow, though with the fix nothing reaches it
//! that way (undoing only that change passes every test). Input that ends
//! early is `error.EndOfStream`, as std means it to be.
//!
//! std's own tests that read its test-data files are left out, since a Zig
//! install need not carry them; `src/core/flate.zig` tests what changed.
...
fn peekBitsEnding(d: *Decompress, n: u4) !u16 {
    ...
    // Changed from std: `left.len * 8 - d.consumed_bits` underflowed once a
    // bad toss (below) had left no byte behind the consumed bits.
    if (left.len * 8 < @as(usize, d.consumed_bits) + n) return error.EndOfStream;
...
fn tossBitsShort(d: *Decompress, n: u4) !void {
    // Changed from std, which added the consumed bits to the bits left
    // rather than taking them away, and so let a code be tossed that ran
    // past the end of the input.
    if (d.input.bufferedLen() * 8 < @as(usize, d.consumed_bits) + n) return error.EndOfStream;
    d.tossBits(n);
}
