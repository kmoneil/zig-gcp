//! Error sets, the mapping from API status to Zig error, and `Diagnostics`.
...
pub const Error = ApiError || TransportError || TokenError || error{
...
/// pass `&diagnostics` in `Client.Options` and read it after a call fails.
/// A call that succeeds clears it. Not safe to share across concurrent tasks.
